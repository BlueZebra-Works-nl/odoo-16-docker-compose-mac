...
``` bash
curl -s https://raw.githubusercontent.com/BlueZebra-Works-nl/odoo-16-docker-compose-mac/master/run.sh | sudo bash -s odoo-one 10016 20016
```
and/or run the following to set up another Odoo instance @ `localhost:11016` (default master password: `minhng.info`):

``` bash
curl -s https://raw.githubusercontent.com/BlueZebra-Works-nl/odoo-16-docker-compose-mac/master/run.sh | sudo bash -s odoo-two 11016 21016
```
